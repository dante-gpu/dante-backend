...
func NewJobConsumer(nc *nats.Conn, cfg *config.Config, prClient *clients.Client, logger *zap.Logger) (*JobConsumer, error) {
...
func (jc *JobConsumer) handleMessage(msg *nats.Msg) {
	...
	jc.logger.Info("Successfully unmarshalled job from NATS",
		zap.String("job_id", internalJob.JobDetails.ID),
		zap.String("job_name", internalJob.JobDetails.Name),
	)

	jc.mu.Lock()
	jc.activeJobs[internalJob.JobDetails.ID] = internalJob
	jc.mu.Unlock()

	scheduled, err := jc.scheduleJob(internalJob)
	if err != nil {
		jc.logger.Error("Scheduling failed for job", zap.String("job_id", internalJob.JobDetails.ID), zap.Error(err))
		// If scheduling itself fails (e.g., provider registry down), Nak for retry
		if nakErr := msg.NakWithDelay(30 * time.Second); nakErr != nil { // Nak with delay
			...
			// If NAK fails, it might be ACKed by timeout or redelivered. Fallback to Ack to avoid loop for now under critical NAK fail.
			_ = msg.Ack() // Or Term if appropriate for unrecoverable NAK issues
		}
		return
	}
	...
	if !scheduled {
		...
		internalJob.State = models.JobStatePending // Or a specific 'waiting_for_resources' state
		internalJob.Attempts++
		internalJob.UpdatedAt = time.Now().UTC()
		// NAK the message so JetStream redelivers it later for another attempt.
		// Implement max attempts logic here if needed.
		if nakErr := msg.NakWithDelay(1 * time.Minute); nakErr != nil { // Nak with a longer delay for resource unavailability
			...
			_ = msg.Ack() // Fallback to Ack to avoid loop under critical NAK fail
		}
		return
	}
	...
	if ackErr := msg.AckSync(); ackErr != nil {
		...
		// If ACK fails, job might be reprocessed. Idempotency in dispatch/provider daemon is crucial.
	}
...
func (jc *JobConsumer) scheduleJob(internalJob *models.InternalJobRepresentation) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("provider registry query failed: %w", err) // Error for retry
	}
	...
	for _, p := range providers {
		...
		// TODO: Add more sophisticated matching: VRAM, specific GPU models within a provider if heterogeneous, etc.
	...
	if suitableProvider == nil {
		...
		return false, nil // No error, just no provider found
	}
	...
	if err != nil {
		...
		internalJob.LastError = "Failed to prepare task data"
		internalJob.UpdatedAt = time.Now().UTC()
		return false, fmt.Errorf("task marshalling failed: %w", err) // Indicate an error for retry
	}

	dispatchSubject := fmt.Sprintf("%s.%s.%s", jc.cfg.NatsTaskDispatchSubjectPrefix, suitableProvider.ID.String(), job.ID)
	jc.logger.Info("Task created, would dispatch to NATS (Placeholder)",
		zap.String("job_id", job.ID),
		zap.String("provider_id", suitableProvider.ID.String()),
	...
	// TODO: Actually publish the task to NATS:
	...
	jc.logger.Info("Job successfully scheduled and (conceptually) dispatched",
		zap.String("job_id", job.ID),
